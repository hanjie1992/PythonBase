...
import homework.ss_06_02_card_management_system.cards_input as cards_input
# 所有名片记录的列表
card_list = []
...
def deal_card(find_dict):
    ...
    if action_str == "1":
        # 调用了input_card_info()函数，传入两个参数，第一个参数find_dict["name"]代表字典中原有的值，
        # 第二个参数代码提示输入的文字，如果输入了直接返回输入的结果，find_dict["name"] =输入的结果，相当于修改了name的值
        ...
        print("{0} 的名片修改成功！".format(find_dict["name"]))
    elif action_str == "2":
